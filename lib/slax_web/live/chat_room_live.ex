defmodule SlaxWeb.ChatRoomLive do
      ...
      <div class="flex flex-col grow shadow-lg">
        <div class="flex justify-between items-center shrink-0 h-16 bg-white border-b border-slate-300 px-4">
          ...
          <ul class="relative z-10 flex items-center gap-4 justify-end">
            <li class="text-sm">{username(@current_scope.user.username)}</li>
        ...
        <div
          :if={!@joined?}
          class="flex justify-around mx-5 mb-5 mb-5 p-6 bg-slate-100 border-slate-300 border rounded-lg"
        >
      ...
      <div class="ml-2">
        <div class="-mt-1">
          <.link class="text-sm font-semibold hover:underline">
            <span>{username(@message.user.username)}</span>
          </.link>
  ...
  defp user(assigns) do
    ~H"""
    <.link href="#" class="flex items-center h-8 hover:bg-gray-300 text-sm pl-8 pr-3">
      ...
      <span class="ml-2 leading-none">{username(@user.username)}</span>
    </.link>
    """
  end

  defp username(user_name) do
    user_name |> String.split("@") |> List.first() |> String.capitalize()
  end
